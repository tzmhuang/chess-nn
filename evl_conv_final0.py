...
def h5_get_conv_x(test,h5_ptr,start,fin):
    ...
    check = h5_ptr['board_checl'][start:fin*64].astype('float16') #(-1,2)
    piece_num = h5_ptr['piece_num'][start:fin*64].astype('float16') #(-1,12)
    current_data = np.concatenate((piece_pos,atk_map,piece_num,turn_move,castling,check), axis = 1)
    del(turn_move,castling,piece_pos,check,piece_num)
    return current_data
...
def model_fn(features, labels, mode):
    #input and reshape
    #input_layer = tf.reshape(features['x'],[-1,8,8,43])
    input_layer = features['x']
    #conv1
    conv1 = tf.layers.conv2d(inputs=input_layer,filters=256,kernel_size=[5, 5],
            padding="same",activation=None,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),name='CONV1')
    ...
    #conv4
    conv3 = tf.layers.conv2d(inputs=relu2, filters=1, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),kernel_size=[1,1 ],
    ...
    dropout = tf.layers.dropout(inputs=relu3, rate=0.5, training=mode == tf.estimator.ModeKeys.TRAIN, name = 'Dropout')
    #tf.summary.histogram('BN4', bn4)
    #dense_layer
    flattern = tf.reshape(dropout, [-1,8*8])
...
with tf.device('/gpu:0'):
    evl_conv_temp = tf.estimator.Estimator(
        model_fn = model_fn, model_dir = "./DNN/evl_conv_final4/")
