...
Purpose:
        - Adamoptimizer
        - L2 regularization
        - Adding Auto Encoder? gradualy decrease tensor size [1544,1000,500,3]
...
Get graph/model:
    gcloud compute copy-files nn-instance1:/home/huangtom2/DNN/evl_Adam_L2reg/ ./
Get model:
    gcloud compute copy-files nn-instance1:/home/huangtom2/DNN/model/... ./
Reset:
    rm -r ./DNN/evl_Adam_L2reg
'''
...
logs_path = "./DNN/evl_Adam_L2reg/graph"
saver_dir = "./DNN/evl_Adam_L2reg/model/evl_Adam_L2reg"
...
def rand_batch(h5_ptr,batch_size, data_size):
    ...
    current_data = np.concatenate((turn_move,castling,piece_pos,flag), axis = 1)
    return current_data
...
with tf.name_scope("input"):
    x = tf.placeholder(tf.float32, name = "input", shape = [None,773])
...
with tf.name_scope("layer_0"):
    with tf.name_scope("weights_0"):
        W_0 = weight_variable(773,773,"W_0")
...
with tf.name_scope("layer_1"):
    with tf.name_scope("weights_1"):
        W_1 = weight_variable(773,1000,"W_1")
...
with tf.name_scope("layer_2"):
    with tf.name_scope("weights_2"):
        W_2 = weight_variable(1000,750,"W_2")
...
with tf.name_scope("layer_3"):
    with tf.name_scope("weights_3"):
        W_3 = weight_variable(750,500,"W_3")
...
with tf.name_scope("cross_entropy"):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits( labels = tf.argmax(y_,1) ,logits = relu_4)
    regularizers = (tf.nn.l2_loss(W_0) + tf.nn.l2_loss(W_1) +
               tf.nn.l2_loss(W_2) + tf.nn.l2_loss(W_3) +
               tf.nn.l2_loss(W_4))
    #loss = tf.reduce_mean(cross_entropy )
    loss = tf.reduce_mean(cross_entropy + 0.0001*regularizers )

tf.summary.scalar("l2_reg",regularizers)
...
with tf.Session() as sess:
    ...
    test_x = test_np[:,0:773]  #testing data
    test_y = test_np[:,773:776] #testing data
    ...
    for epochs in range(training_epochs):
        ...
        for i in range(batch_count):
            ...
            train_x = batch[:,0:773]    #training data
            train_y = batch[:,773:776]   #training flag
