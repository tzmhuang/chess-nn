'''
1. Fully connected Neural network with 3 layers
2. output 3 scores, the score of win, draw, close
3. input 3 scores to softmax function -> get probability
4. alter the result of original data into 3 column
5. use cross cross_entropy as loss function
6. train the  network using batch steepest decend
7. save the trained weights into h5py data file


PROBLEM:
    1. nan values in cost and in training accuracy
    2. non-converging cost and training accuracy
    3. test accuracy < 0.5
'''
...
Get graph:
    gcloud compute copy-files nn-instance1:/home/huangtom2/DNN/graph/evl_NN_2_1/* ./graph/evl_NN_2_1
Get model:
...
#config
logs_path = "./chess_nn/evl_NN_2_2/graph"
saver_dir = "./chess_nn/evl_NN_2_2/model/evl_NN_2_2"
# logs_path = "./DNN/graph/evl_NN_2_1"
# saver_dir = "./DNN/model/evl_NN_2_1"
batch_size = 10000
training_epochs = 1
...
def weight_variable(r_num, c_num, name):
    ...
    return tf.Variable(initial*0.1, name = name)
...
with tf.Session() as sess:
    ...
    for epochs in range(training_epochs):
        ...
        for i in range(batch_count):
            batch = rand_batch(train_h, batch_size, 3406526)
            ...
            if (i)%50 == 0:
                ...
                saver.save(sess, saver_dir, global_step=global_step ,write_meta_graph=False)
        # if (epochs)%5 == 0:
