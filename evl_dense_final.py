...
tf.app.flags.DEFINE_string('model_dir',"./DNN/evl_dense_final/",'dir of model stored' )
...
def h5_get(h5_ptr,start,fin):
    ...
    turn_move = h5_ptr['turn_move'][start:fin].astype('float32') #1
    #turn_move_conv = h5_ptr['turn_move_conv'][start:fin]
    castling = h5_ptr['castling'][start:fin].astype('float32') #4
    board_check = h5_ptr['board_check'][start:fin].astype('float32')#2
    piece_num = h5_ptr['piece_num'][start:fin].astype('float32') #12
    ...
    piece_pos = h5_ptr['piece_pos'][start:fin].astype('float32') #768   (12,8,8)
    atk_map = h5_ptr['atk_map'][start:fin].astype('float32') #768
    flag = h5_ptr['flag'][start:fin].astype('float32') #3
...
def model_fn(features, labels, mode):
    ...
    tf.summary.histogram('win_prob' ,predictions['probabilities'][0][0])
    tf.summary.histogram( 'lose_prob' ,predictions['probabilities'][0][1])
    tf.summary.histogram( 'draw_prob' ,predictions['probabilities'][0][2])
    ...
    with tf.name_scope("Loss"):
        ...
        l2_loss = 0.1*(tf.nn.l2_loss(W_0)+tf.nn.l2_loss(W_1)+tf.nn.l2_loss(W_2))
...
data = np.concatenate((h5_get(train_h,0,data_size[0]),h5_get(test_h,0,data_size[1])), axis = 0)
...
data_x = data[train_index,0:1555]
data_y = data[train_index,1555:1558]
...
with tf.device('/gpu:0'):
    evl_conv_temp = tf.estimator.Estimator(
        model_fn = model_fn, model_dir ="./DNN/evl_dense_final/")

#evl_conv_temp.train(
#    input_fn = train_input_fn,hooks = [logging_hook])


eval_x = data[dev_index,0:1555]
eval_y = data[dev_index,1555:1558]
test_x = data[test_index,0:1555]
test_y = data[test_index,1555:1558]
