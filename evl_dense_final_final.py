...
def model_fn(features, labels, mode):
    ...
    with tf.name_scope("layer_1"):
        with tf.name_scope("weights_1"):
            W_1 = weight_variable(1555,1555,"W_1")
        ...
        with tf.name_scope("pre_activate_1"):
            a_1 = full_layer(input_layer, W_1, b_1)
    ...
    with tf.name_scope("layer_2"):
        with tf.name_scope("weights_2"):
            W_2 = weight_variable(1555,1555,"W_2")
    ...
    with tf.name_scope("logits"):
        with tf.name_scope("weights_4"):
            W_4 = weight_variable(1555,3,"W_4")
    ...
    with tf.name_scope("Loss"):
        ...
        l2_loss = 0.01*(tf.nn.l2_loss(W_1)+tf.nn.l2_loss(W_2)+tf.nn.l2_loss(W_4))
...
with tf.device('/gpu:0'):
    evl_conv_temp = tf.estimator.Estimator(
        model_fn = model_fn, model_dir ="./DNN/evl_dense_final0/")
