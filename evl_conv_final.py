...
partition_train = int(0.8*sum(data_size))
partition_dev = int(0.1*sum(data_size))
partition_test = sum(data_size) - partition_train - partition_dev

batch_size = 1024
training_epochs = 30

FLAGS = tf.app.flags.FLAGS
tf.app.flags.DEFINE_string('model_dir',"./DNN/evl_conv_final/",'dir of model stored' )
...
def h5_get_conv_x(test,h5_ptr,start,fin):
    #move_num = h5_ptr['move_num'][pos:pos+batch_size] #1
    #game_phase = h5_ptr['game_phase'][ind] #3
    #turn_move = h5_ptr['turn_move'][ind] #1
    if test:
        turn_move = h5_ptr['turn_move_new'][start:fin*64].astype('float16')
    else:
        turn_move = h5_ptr['turn_move'][start:fin*64].astype('float16')
    #castling = h5_ptr['castling'][ind] #4
    castling = h5_ptr['castling'][start:fin*64].astype('float16') #(-1,4)
    board_set = h5_ptr['board_set'][start:fin*64].astype('float16') #64 (-1,1)
    piece_pos = h5_ptr['piece_pos'][start:fin*64].astype('float16') #768   (-1,12)
    atk_map = h5_ptr['atk_map'][start:fin*64].astype('float16') #768 (-1,12)
    current_data = np.concatenate((board_set,piece_pos,atk_map,turn_move,castling), axis = 1)
    del(turn_move,castling,piece_pos)
    return current_data

def h5_get_conv_y(h5_ptr,start,fin):
    flag = h5_ptr['flag'][start:fin].astype('float16') #3 (-1,3)
    return flag
...
def model_fn(features, labels, mode):
    ...
    #conv1
    conv1 = tf.layers.conv2d(inputs=input_layer,filters=256,kernel_size=[5, 5],
    ...
    conv2 = tf.layers.conv2d(inputs=relu1, filters=256, kernel_size=[3, 3],
            padding="same", activation=None,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),name='CONV2')
    ...
    #conv3
    conv3 = tf.layers.conv2d(inputs=relu2, filters=256, kernel_size=[3, 3],
            padding="same", activation=None,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),name='CONV3')
    ...
    #conv4
    conv4 = tf.layers.conv2d(inputs=relu3, filters=256, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),kernel_size=[3,3 ],
    ...
    #conv_5
    conv5 = tf.layers.conv2d(inputs=relu4, filters=1, kernel_size=[1, 1],
            padding="same", activation=None,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001),name='CONV5')
    bn5 = tf.layers.batch_normalization(
            inputs = conv5, training= mode==tf.estimator.ModeKeys.TRAIN, name = 'BN5')
    relu5 = tf.nn.relu(bn5, name = 'relu5')
    dropout = tf.layers.dropout(inputs=bn5, rate=0.6, training=mode == tf.estimator.ModeKeys.TRAIN, name = 'Dropout')
    tf.summary.histogram('conv5', conv5)
    tf.summary.histogram('relu5', relu5)
    #tf.summary.histogram('BN4', bn4)
    #dense_layer
    flattern = tf.reshape(relu5, [-1,8*8])
    dense_1 = tf.layers.dense(inputs=flattern, units=64,kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001), name='DENSE_1')
...
#data_x = h5_get_conv_x(False,train_h,0,data_size[0])

data_index = np.array(range(sum(data_size)))
np.random.seed(2018)
np.random.shuffle(data_index)
train_index = list(data_index[0:partition_train])
dev_index = list(data_index[partition_train:partition_train+partition_dev])
test_index = list(data_index[partition_train+partition_dev: sum(data_size)])

#reads in float16
data_x = np.concatenate((h5_get_conv_x(False,train_h,0,data_size[0]),h5_get_conv_x(True,test_h,0,data_size[1]))
        ,axis = 0).reshape(-1,8,8,30)

train_x = data_x[train_index,:,:,:].astype('float32')
eval_x =  data_x[dev_index,:,:,:].astype('float32')
test_x = data_x[test_index,:,:,:].astype('float32')
del(data_x)



data_y = np.concatenate((h5_get_conv_y(train_h,0,data_size[0]),h5_get_conv_y(test_h,0,data_size[1])), axis = 0)
train_y = data_y[train_index,:].astype('float32')
test_y = data_y[test_index,:].astype('float32')
eval_y= data_y[dev_index,:].astype('float32')
...
with tf.device('/gpu:0'):
    evl_conv_temp = tf.estimator.Estimator(
        model_fn = model_fn, model_dir = "./DNN/evl_conv_final/")
...
print('==================testing==================')
test_results = evl_conv_temp.evaluate(input_fn=test_input_fn)
