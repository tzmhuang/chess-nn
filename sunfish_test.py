...
def main():
    ...
    while True:
        ...
        while move not in pos.gen_moves():
            ...
            board.push(my_move)
            my_move = my_move.__str__()
            print(my_move)
            print ('########')
            print ('time used:', time.time()-start_time)
            print ('########')
            moves_list.append(my_move)
            match = re.match('([a-h][1-8])'*2, my_move )
            if match:
        ...
        pos = pos.move(move)
        its_move = render(119-move[0]) + render(119-move[1])
        board.push(chess.Move.from_uci(its_move))
        moves_list.append(its_move)
